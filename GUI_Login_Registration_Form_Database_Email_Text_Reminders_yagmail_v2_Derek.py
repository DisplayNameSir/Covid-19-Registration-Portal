...
Twilio_Number="+18883016704", #this is your own Twilio number
account_sid = 'AC6e5ddbbc08048c697058226731d6675f'  #this is your account SID
auth_token = 'b6eb2d45015723d2bb8c0b09dd3526f6'     #this is your own auth_token
...
def excel():
    ...
    #Added titles for user inputs - Isagi
    sheet.cell(row=1, column=12).value = "SocialSecurity"
    sheet.cell(row=1, column=13).value = "EmergencyContact"
    sheet.cell(row=1, column=14).value = "VaccinationType"
...
#Setting focus for new fields - Isagi
def focus8(event):
    social_field.focus_set()

def focus9(event):
    emergency_field.focus_set()

def focus10(event):
    vacctype_field.focus_set()
...
def clear():
    ...
    #Added cleared content - Isagi
    social_field.delete(0, END)
    emergency_field.delete(0, END)
    vacctype_field.delete(0, END)
...
def insert():
    ...
    if (
        ...
        #if statement for new fields - Isagi
        or social_field.get() == ""
        or emergency_field.get() == ""
        or vacctype_field.get() == ""
    ):
        Empty_Input()
        ...
        #Added user input - Isagi
        sheet.cell(row=current_row + 1, column=12).value = social_field.get()
        sheet.cell(row=current_row + 1, column=13).value = emergency_field.get()
        sheet.cell(row=current_row + 1, column=14).value = vacctype_field.get()
...
def register_form():
    ...
    #Added global variables - Isagi
    global social_field
    global emergency_field
    global vacctype_field
    ...
    #Adding labels to registration page - Isagi
    sos = Label(root, text="Social Security", bg="black", fg="white", font=("Calibri", 13))
    emega = Label(root, text="Emergency Contact", bg="black", fg="white", font=("Calibri", 13))
    facc = Label(root, text="Vaccination Brand", bg="black", fg="white", font=("Calibri", 13))
    ...
    #Placing labels into proper positions - Isagi
    sos.grid(row=9, column=0)
    emega.grid(row=10, column=0)
    facc.grid(row=11, column=0)
    ...
    #Adding Entry Roots Also have to add textvariables - Isagi
    social_field = Entry(root, textvariable=sos_in)
    emergency_field = Entry(root, textvariable=emega_in)
    vacctype_field = Entry(root, textvariable=facc_in)
    ...
    #Added Field Binds - Isagi
    social_field.bind("<Return>", focus8)
    emergency_field.bind("<Return>", focus9)
    vacctype_field.bind("<Return>", focus10)
    ...
    #Added text entry boxes - Isagi
    social_field.grid(row=9, column=1, ipadx="100")
    emergency_field.grid(row=10, column=1, ipadx="100")
    vacctype_field.grid(row=11, column=1, ipadx="100")
    ...
    submit.grid(row=12, column=1) #Experiment - Isagi
...
def register():
    ...
    # refrenced above
    bg = PhotoImage(file=r"pyimage2.png", height=800, width=800)
...
def login():
    ...
    bg = PhotoImage(file=r"BP1.png", height=400, width=320)
...
def main_account_screen():
    ...
    # Import and set ground of main screen Also changed pictures/colors - Isagi
    ...
    Label(
        text="Welcome to SJSU COVID-19 Vaccination Registration/Alerts Portal!",
        fg="orange",
        bg="black", #all RGB = 0000FF
        width="57",
        height="3",
        font=("Calibri", 20),
